...
<template>
    <div>
        <div class="d-flex justify-space-between align-center flex-wrap gap-y-4 mb-6">
            <div>
                <div class="d-flex gap-2 align-center mb-2 flex-wrap">
                    ...
                    <div class="d-flex gap-x-2">
                        <VChip v-if="orderData?.payment" variant="tonal"
                            :color="resolvePaymentStatus(orderData.payment)?.color" size="small">
                            {{ resolvePaymentStatus(orderData.payment)?.text }}
                        </VChip>
                        <VChip v-if="orderData?.status" v-bind="resolveStatus(orderData?.status)" size="small" />
                    </div>
                </div>
                <div>
            ...
            <VBtn variant="outlined" color="error" @click="isConfirmDialogVisible = !isConfirmDialogVisible">
                Delete Order
            </VBtn>
        </div>
        ...
        <VRow>
            <VCol cols="12" md="8">
                ...
                <VCard class="mb-6">
                    ...
                    <VDataTable :headers="headers" :items="orderDetail" item-value="productName" show-select
                        class="text-no-wrap">
                        <template #item.productName="{ item }">
                            <div class="d-flex gap-x-3">
                                <VAvatar size="34" variant="tonal" :image="item.productImage" rounded />
                ...
                <VCard class="mb-6">
                    <VCardText class="d-flex flex-column gap-y-6">
                        ...
                        <div class="d-flex align-center">
                            <VAvatar v-if="orderData" size="34"
                                :variant="!orderData?.avatar.length ? 'tonal' : undefined" :rounded="1" class="me-3">
                        ...
                        <div class="d-flex flex-column gap-y-1">
                            <div class="d-flex justify-space-between gap-1 text-body-2">
                                ...
                                <span class="text-base text-primary font-weight-medium cursor-pointer"
                ...
                <VCard class="mb-6">
                    <VCardText>
                        <div class="d-flex align-center justify-space-between gap-1 mb-6">
                            ...
                            <span class="text-base text-primary font-weight-medium cursor-pointer"
                                @click="isEditAddressDialogVisible = !isEditAddressDialogVisible">Edit</span>
                        </div>
                        <div>
        ...
        <ConfirmDialog v-model:isDialogVisible="isConfirmDialogVisible"
            confirmation-question="Are you sure to cancel your Order?" cancel-msg="Order cancelled!!"
            cancel-title="Cancelled" confirm-msg="Your order cancelled successfully." confirm-title="Cancelled!" />
