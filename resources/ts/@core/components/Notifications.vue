...
<template>
    <IconBtn id="notification-btn">
        <VBadge dot v-bind="props.badgeProps" :model-value="props.notifications.some(n => !n.isSeen)" color="error"
            bordered offset-x="1" offset-y="1">
            <VIcon icon="ri-notification-2-line" />
        </VBadge>

        <VMenu activator="parent" width="380" :location="props.location" offset="15px" :close-on-content-click="false">
            <VCard class="d-flex flex-column">
                ...
                <VCardItem class="notification-section">
                    ...
                    <template #append>
                        <VChip v-show="!!isAllMarkRead" size="small" class="me-3" variant="tonal" color="primary">
                            {{ totalUnreadNotifications }} new
                        </VChip>

                        <IconBtn v-show="props.notifications.length" @click="markAllReadOrUnread">
                            <VIcon color="high-emphasis"
                ...
                <PerfectScrollbar :options="{ wheelPropagation: false, suppressScrollX: true }"
                    ...
                    <VList class="py-0">
                        <template v-for="(notification, index) in props.notifications" :key="notification.title">
                            ...
                            <!-- For task comment added notification -->
                            <template v-if="notification.type === 'task.comment.added'">
                                <VListItem link lines="one" min-height="66px" class="list-item-hover-class py-3"
                                    @click="$emit('click:notification', notification)">
                                    <!-- Slot: Prepend -->
                                    <!-- Handles Avatar: Image, Icon, Text -->
                                    <div class="d-flex align-start gap-3">
                                        <div>
                                            <VAvatar class="cursor-pointer" size="38" variant="tonal" color="primary">
                                                {{ getInitials(notification.comment.createdBy.name) }}
                                            </VAvatar>
                                        </div>

                                        <div>
                                            <h6 class="text-h6 mb-1">
                                                New task comment added!
                                            </h6>
                                            <p class="text-body-2 mb-2"
                                                style="letter-spacing: 0.4px !important; line-height: 18px;">
                                                {{ notification.comment.createdBy.name }} has added a new comment added
                                                in task "{{ notification.comment.task.title }}".
                                            </p>
                                            <p class="text-sm text-disabled mb-0"
                                                style="letter-spacing: 0.4px !important; line-height: 18px;">
                                                {{ moment(notification.created_at).fromNow() }}
