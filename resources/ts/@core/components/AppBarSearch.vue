...
<template>
    <VDialog max-width="600" :model-value="props.isDialogVisible" :height="$vuetify.display.smAndUp ? '537' : '100%'"
        ...
        <VCard height="100%" width="100%" class="position-relative">
            <VCardText class="py-3 px-4">
                <!-- 👉 Search Input -->
                <VTextField ref="refSearchInput" v-model="searchQueryLocal" autofocus density="compact" variant="plain"
                    class="app-bar-search-input" @keyup.esc="clearSearchAndCloseDialog" @keydown="getFocusOnSearchList"
                    @update:model-value="$emit('search', searchQueryLocal)">
                    <!-- 👉 Prepend Inner -->
                    <template #prepend-inner>
                        <div class="d-flex align-center text-high-emphasis me-1">
                            <VIcon size="24" icon="ri-search-line" style=" margin-block-start: 1px; opacity: 1;" />
                        </div>
                    </template>
                    ...
                    <template #append-inner>
                        <div class="d-flex align-start">
                            ...
                            <IconBtn class="mt-n2" color="medium-emphasis" @click="clearSearchAndCloseDialog">
            ...
            <PerfectScrollbar :options="{ wheelPropagation: false, suppressScrollX: true }" class="h-100">
                ...
                <VList v-show="searchQueryLocal.length && !!props.searchResults.length" ref="refSearchList"
                    density="compact" class="app-bar-search-list py-0">
                    ...
                    <slot name="noData">
                        <VCardText class="h-100">
                            <div
                                ...
                                <VIcon size="64" icon="ri-file-forbid-line" />
